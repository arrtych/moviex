...
export function Toaster() {
  ...
  const getToastIcon = (title?: React.ReactNode, variant?: string) => {
    ...
    if (typeof title === "string") {
      ...
      if (titleLower.includes("paused"))
        return <Pause className="h-5 w-5 text-amber-500 mr-2" />;
      if (titleLower.includes("resumed"))
        return <Play className="h-5 w-5 text-green-500 mr-2" />;
