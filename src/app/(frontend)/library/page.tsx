...
export default function LibraryPage() {
  ...
  const filteredLibraryItems = libraryItems.filter((item: LibraryItem) => {
    ...
    const storageMatch =
      activeStorageFilter === 'all' ? true : item.storageId === activeStorageFilter
    ...
    <div className="container mx-auto px-4 py-8 pt-24">
      ...
      <ScrollableFilters className="mb-4">
        ...
        <div className="flex items-center space-x-2">
          ...
          <Tabs
            defaultValue="all"
            value={activeStorageFilter}
            onValueChange={setActiveStorageFilter}
            className="h-7"
          >
            <TabsList className="h-7 p-1">
              <TabsTrigger value="all" className="text-xs h-5 px-2">
              ...
              {storages.map((storage: Storage) => (
                <TabsTrigger
                  key={storage.id}
                  value={storage.id}
                  className="text-xs h-5 px-2 flex items-center"
                >
      ...
      <ScrollableFilters className="mb-4">
        {storages.map((storage: Storage) => (
          <Card
            key={storage.id}
            className={`overflow-hidden transition-all hover:shadow-md flex-shrink-0 w-48 mr-2 ${
              activeStorageFilter === storage.id ? 'ring-1 ring-primary' : ''
            }`}
                ...
                <>
                  <div className="h-1 bg-muted rounded-full mb-1">
                    <div
                      ...
                      style={{
                        width: `${
                          ((storage.totalSpace - storage.freeSpace) / storage.totalSpace) * 100
                        }%`,
                      }}
                    ></div>
                  </div>
                  ...
                  <div className="flex flex-col md:flex-row">
                    ...
                    <div className="p-3 flex-1">
                      ...
                      <div className="mt-2">
                        ...
                        {item.status !== 'downloaded' && (
                          <Progress value={downloadProgress} className="h-1.5 mb-2" />
                        )}
