...
export const MovieProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const addToFavorites = (movieId: string, type: 'bookmark' | 'like' | 'watchLater') => {
    ...
    if (existingFavorite) {
      setFavorites(
        favorites.map((fav) => (fav.movieId === movieId ? { ...fav, [type]: true } : fav)),
      )
    } else {
      setFavorites([
        ...favorites,
  ...
  const removeFromFavorites = (movieId: string, type: 'bookmark' | 'like' | 'watchLater') => {
    ...
    if (existingFavorite) {
      ...
      if (
        ...
        setFavorites(
          favorites.map((fav) => (fav.movieId === movieId ? { ...fav, [type]: false } : fav)),
        )
      }
    }
  }
      ...
      setWatchHistory(
        watchHistory.map((entry) =>
          entry.movieId === movieId
            ? { ...entry, watchedAt: now, progress: Math.min(entry.progress + 0.1, 1) }
            : entry,
        ),
      )
    } else {
