...
export default function FavoritesPage() {
  ...
  const bookmarkedMovies = movies.filter((movie) => favorites.some((fav) => fav.movieId === movie.id && fav.bookmark))
  const likedMovies = movies.filter((movie) => favorites.some((fav) => fav.movieId === movie.id && fav.like))
  const allFavoriteMovies = movies.filter((movie) => favorites.some((fav) => fav.movieId === movie.id))
      ...
      <>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-2">
          {displayedMovies.map((movie) => (
            <FilmCard key={movie.id} movie={movie} activeId={activeId || undefined} onActivate={handleActivate} />
          ))}
        </div>
    ...
    <div className="container mx-auto px-4 py-8 pt-24">
      ...
      <Tabs defaultValue="all">
        ...
        <TabsContent value="all">
          {allFavoriteMovies.length === 0 ? renderEmptyState("Favorite") : renderMovieGrid(allFavoriteMovies)}
        </TabsContent>

        <TabsContent value="bookmarked">
          {bookmarkedMovies.length === 0 ? renderEmptyState("Bookmarked") : renderMovieGrid(bookmarkedMovies)}
        </TabsContent>
