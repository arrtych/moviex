...
export default function DirectorsPage() {
  ...
  const filteredDirectors = directors.filter((director: Director) => {
    return (
      ...
      director.knownFor.some((movie) => movie.toLowerCase().includes(searchQuery.toLowerCase()))
    )
  })

  // Sort the filtered directors
  ...
  // Get directors to display based on limit
  const displayedDirectors = sortedDirectors.slice(0, displayLimit)
  const hasMore = displayedDirectors.length < sortedDirectors.length
      ...
      {filteredDirectors.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-muted-foreground">
            We couldn't find any directors matching your search criteria. Try adjusting your search terms.
          </p>
        </div>
      ) : (
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {displayedDirectors.map((director: Director) => (
              <Link key={director.id} href={`/directors/${director.slug}`}>
                <Card className="overflow-hidden transition-all hover:shadow-md hover:scale-105">
                  <CardContent className="p-0">
                    <div className="relative aspect-[3/4]">
                      <Image
                        src={director.profileImage || "/placeholder.svg?height=400&width=300"}
                        alt={director.name}
                        fill
                      ...
                      <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-3">
                        <div className="flex items-center">
                          ...
                          <span className="text-xs text-white">{director.popularity.toFixed(1)}</span>
                        </div>
                      </div>
                    </div>
                    <div className="p-3">
                      <h3 className="font-medium text-sm line-clamp-1">{director.name}</h3>
                      <p className="text-xs text-muted-foreground">
                        Age: {calculateAge(director.birthDate, director.deathDate)}
                      </p>
                    </div>
                  </CardContent>
                </Card>
