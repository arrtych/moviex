  ...
  return (
    <>
      <Link
        ...
        className={`block relative rounded-lg overflow-hidden group ${isActive ? "movie-card-active" : "movie-card-hover"} max-w-[${variant === "large" ? "192px" : variant === "default" ? "160px" : "128px"}] ${!isInLibrary && !isActive ? "opacity-75" : "opacity-100"}`}
      ...
      >
        <div className={`relative ${cardSizes[variant]}`}>
          <Image
            src={
              movie.posterPath ||
              `/placeholder.svg?height=${variant === "large" ? 288 : variant === "default" ? 240 : 192}&width=${variant === "large" ? 192 : variant === "default" ? "160px" : "128px"}`
            }
            alt={movie.title}
            fill
            className="object-cover rounded-lg"
            sizes={`(max-width: 768px) 100vw, ${variant === "large" ? "192px" : variant === "default" ? "160px" : "128px"}`}
          />
          ...
          {movie.labels && movie.labels.length > 0 && (
            <div className="absolute top-2 left-2 flex flex-col gap-1">
              {movie.labels.map((label) => (
                <span
                  key={label.slug}
                  className={`${labelSize === "xs" ? "text-[0.6rem] px-1 py-0.5" : "text-xs px-1.5 py-0.5"} font-bold rounded ${label.color} text-white shadow-md`}
                >
                  {label.text}
                </span>
              ))}
          ...
          >
            ...
            <div className="space-y-1">
              ...
              <div className="flex items-center justify-between">
                <span className="text-xs text-white">{new Date(movie.releaseDate).getFullYear()}</span>
        ...
        <div className="mt-2 px-1">
          ...
          <div className="flex items-center justify-between">
            ...
            {isSerial && <span className="text-xs bg-primary/10 text-primary px-1 rounded">TV</span>}
          </div>
        </div>
      </Link>
